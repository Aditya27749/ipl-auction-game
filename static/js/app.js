...
// --- Audio Engine ---
let fxAudioCtx = null;
try { fxAudioCtx = new (window.AudioContext || window.webkitAudioContext)(); } catch(e){}
document.body.addEventListener('click', () => {
    if (fxAudioCtx && fxAudioCtx.state === 'suspended') fxAudioCtx.resume();
}, { once: true });
...
const sounds = {
    playBid: function() {
        ...
        const gainNode = fxAudioCtx.createGain();
        osc.connect(gainNode);
        gainNode.connect(fxAudioCtx.destination);
        osc.type = 'sine';
        osc.frequency.setValueAtTime(800, fxAudioCtx.currentTime);
        osc.frequency.exponentialRampToValueAtTime(1200, fxAudioCtx.currentTime + 0.1);
        gainNode.gain.setValueAtTime(0.1, fxAudioCtx.currentTime);
        gainNode.gain.exponentialRampToValueAtTime(0.01, fxAudioCtx.currentTime + 0.1);
        osc.start();
        osc.stop(fxAudioCtx.currentTime + 0.1);
    },
    playSold: function() {
        if (!fxAudioCtx || fxAudioCtx.state === 'suspended') return;
        const osc = fxAudioCtx.createOscillator();
        ...
        gainNode.connect(fxAudioCtx.destination);
        osc.type = 'triangle';
        osc.frequency.setValueAtTime(150, fxAudioCtx.currentTime);
        osc.frequency.exponentialRampToValueAtTime(40, fxAudioCtx.currentTime + 0.2);
        gainNode.gain.setValueAtTime(0.5, fxAudioCtx.currentTime);
        gainNode.gain.exponentialRampToValueAtTime(0.01, fxAudioCtx.currentTime + 0.2);
        osc.start();
        osc.stop(fxAudioCtx.currentTime + 0.2);
    },
    ...
    playHeartbeat: function() {
        ...
        gainNode.connect(fxAudioCtx.destination);
        osc.type = 'sine';
        osc.frequency.setValueAtTime(50, fxAudioCtx.currentTime);
        osc.frequency.exponentialRampToValueAtTime(30, fxAudioCtx.currentTime + 0.1);
        gainNode.gain.setValueAtTime(1, fxAudioCtx.currentTime);
        gainNode.gain.exponentialRampToValueAtTime(0.01, fxAudioCtx.currentTime + 0.4);
        osc.start();
        osc.stop(fxAudioCtx.currentTime + 0.4);
        
        const osc2 = fxAudioCtx.createOscillator();
        const gain2 = fxAudioCtx.createGain();
        osc2.connect(gain2);
        gain2.connect(fxAudioCtx.destination);
        osc2.type = 'sine';
        osc2.frequency.setValueAtTime(50, fxAudioCtx.currentTime + 0.15);
        osc2.frequency.exponentialRampToValueAtTime(30, fxAudioCtx.currentTime + 0.25);
        gain2.gain.setValueAtTime(0.8, fxAudioCtx.currentTime + 0.15);
        gain2.gain.exponentialRampToValueAtTime(0.01, fxAudioCtx.currentTime + 0.5);
        osc2.start(fxAudioCtx.currentTime + 0.15);
        osc2.stop(fxAudioCtx.currentTime + 0.5);
    },
    playShredder: function() {
        if (!fxAudioCtx || fxAudioCtx.state === 'suspended') return;
        const bufferSize = fxAudioCtx.sampleRate * 2.0; // 2 seconds of noise
        const buffer = fxAudioCtx.createBuffer(1, bufferSize, fxAudioCtx.sampleRate);
        const data = buffer.getChannelData(0);
        for (let i = 0; i < bufferSize; i++) {
            data[i] = Math.random() * 2 - 1;
        }
        const noise = fxAudioCtx.createBufferSource();
        noise.buffer = buffer;
        
        const filter = fxAudioCtx.createBiquadFilter();
        filter.type = 'lowpass';
        filter.frequency.setValueAtTime(500, fxAudioCtx.currentTime);
        filter.frequency.linearRampToValueAtTime(100, fxAudioCtx.currentTime + 2.0);
        
        const gain = fxAudioCtx.createGain();
        gain.gain.setValueAtTime(0.5, fxAudioCtx.currentTime);
        gain.gain.exponentialRampToValueAtTime(0.01, fxAudioCtx.currentTime + 2.0);
    ...
    playTick: function() {
        ...
        gain.connect(fxAudioCtx.destination);
        osc.type = 'square';
        osc.frequency.setValueAtTime(400, fxAudioCtx.currentTime);
        gain.gain.setValueAtTime(0.05, fxAudioCtx.currentTime);
        gain.gain.exponentialRampToValueAtTime(0.001, fxAudioCtx.currentTime + 0.05);
        osc.start();
    ...
    playSuccess: function() {
        ...
        gainNode.connect(fxAudioCtx.destination);
        osc.type = 'sine';
        osc.frequency.setValueAtTime(1000, fxAudioCtx.currentTime);
        osc.frequency.setValueAtTime(1500, fxAudioCtx.currentTime + 0.1);
        gainNode.gain.setValueAtTime(0.1, fxAudioCtx.currentTime);
        gainNode.gain.linearRampToValueAtTime(0, fxAudioCtx.currentTime + 0.3);
        osc.start();
        osc.stop(fxAudioCtx.currentTime + 0.3);
    }
};
